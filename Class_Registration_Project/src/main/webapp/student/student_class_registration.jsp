...
<section id="section1">
    <form method="get">
        <select name="searchField"> <!--select 태그는 선택한 option에 따라 option의 value가 전송됨-->
...
<section id="section2">
    <table>
        ...
        <tbody>
        <%
            for (SearchDTO searchDTO : list) { //반복문 돌리면서 튜플 출력
        %>
        <tr>
            <td>
            ...
            <td>
                <form action="student_wish_add.jsp"> <!--희망 내역에 추가하기 위해 필요한 변수들 넘김-->
                    <input type="hidden" name="class_id" value="<%=searchDTO.getClassId()%>">
                    <input type="hidden" name="student_id" value="<%=session.getAttribute("user_id")%>">
                    <button type="submit">희망</button>
                </form>
